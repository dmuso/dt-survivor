...
/// Marker component for the whisper's 3D model entity (child of the Whisper entity)
#[derive(Component)]
pub struct WhisperModel;

/// Marker component for the whisper's AnimationPlayer entity
#[derive(Component)]
pub struct WhisperAnimationPlayer;
...
mod tests {
    ...
    fn test_whisper_companion_default() {
        ...
        assert_eq!(companion.follow_offset, Vec3::new(0.0, 4.0, 0.0));
        assert_eq!(companion.bob_phase, 0.0);
        assert_eq!(companion.bob_amplitude, 0.15);
    }
    ...
    fn test_lightning_bolt_new_full_size() {
        ...
        assert_eq!(bolt.max_distance, 32.0);
        assert_eq!(bolt.speed, 7.0);
        ...
        assert_eq!(bolt.jag_offsets.len(), 6);
        assert_eq!(bolt.joint_distances.len(), 6);
        assert_eq!(bolt.base_thickness, 1.5);
        assert_eq!(bolt.tip_thickness, 0.5);
    }
    ...
    fn test_lightning_bolt_size_multiplier() {
        ...
        assert_eq!(half_bolt.max_distance, 16.0);
        assert_eq!(half_bolt.base_thickness, 0.75);
        assert_eq!(half_bolt.tip_thickness, 0.25);

        let min_bolt = LightningBolt::new(0.0, 42, Vec3::ZERO, max_dist, 0.2);
        assert!((min_bolt.max_distance - 6.4).abs() < 0.001);
        assert!((min_bolt.base_thickness - 0.3).abs() < 0.001);
        assert!((min_bolt.tip_thickness - 0.1).abs() < 0.001);
    }
    ...
    #[test]
    fn test_whisper_model_marker() {
        let _marker = WhisperModel;
    }

    #[test]
    fn test_whisper_animation_player_marker() {
        let _marker = WhisperAnimationPlayer;
    }
}
