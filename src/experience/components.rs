...
mod tests {
    ...
    fn xp_to_next_level_at_level_one() {
        let exp = PlayerExperience::new();
        // base_xp=100, multiplier=1.5, level=1
        // 100 * 1.5^0 = 100
    ...
    fn xp_to_next_level_follows_multiplier_formula() {
        ...
        assert_eq!(exp.xp_to_next_level(), 150);

        exp.level = 3;
        // Level 3: 100 * 1.5^2 = 225
        assert_eq!(exp.xp_to_next_level(), 225);

        exp.level = 4;
        // Level 4: 100 * 1.5^3 = 337.5 -> ceil = 338
        assert_eq!(exp.xp_to_next_level(), 338);
    }

    #[test]
    fn add_xp_accumulates_without_level_up() {
        let mut exp = PlayerExperience::new();
        let levels = exp.add_xp(50);
        assert_eq!(levels, 0);
        assert_eq!(exp.current, 50);
        assert_eq!(exp.total_xp, 50);
        assert_eq!(exp.level, 1);
    }
    ...
    fn add_xp_handles_multiple_level_ups() {
        ...
        // Level 1 -> 2: 100 XP
        // Level 2 -> 3: 150 XP
        // Total for 2 levels: 250 XP
        let levels = exp.add_xp(260);
        assert_eq!(levels, 2);
        assert_eq!(exp.level, 3);
        assert_eq!(exp.current, 10);
        assert_eq!(exp.total_xp, 260);
    }

    #[test]
    fn add_xp_handles_many_level_ups() {
        let mut exp = PlayerExperience::new();
        let levels = exp.add_xp(1000);
        assert!(levels > 2, "1000 XP should grant more than 2 levels");
        assert!(exp.level > 3, "Should be above level 3");
        assert_eq!(exp.total_xp, 1000);
    }
    ...
    fn progress_returns_correct_percentage() {
        let mut exp = PlayerExperience::new();
        exp.current = 50;
        // With xp_to_next_level=100, progress should be 0.5
    ...
    fn progress_near_level_up_is_near_one() {
        let mut exp = PlayerExperience::new();
        exp.current = 99;
        // With xp_to_next_level=100, progress should be 0.99
