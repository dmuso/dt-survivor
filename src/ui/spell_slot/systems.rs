...
use crate::ui::spell_slot::components::{
    LevelIndicatorContainer, SlotSource, SpellIconImage, SpellLevelIndicator, SpellSlotVisual,
};
...
///
/// This system is the single source of truth for spell slot visuals:
/// - Spell present: load texture (custom or default), show level indicator, element background
...
pub fn refresh_spell_slot_visuals(
    ...
    // Update level indicator container visibility
...
mod tests {
    ...
    mod refresh_spell_slot_visuals_tests {
        ...
        #[test]
        fn slot_with_spell_has_empty_slot_background() {
            ...
            // Background should be empty slot color - element shown via texture
            let (bg, _) = app
                .world_mut()
                .query::<(&BackgroundColor, &SpellSlotVisual)>()
                .iter(app.world())
        ...
        fn updates_multiple_slots_correctly() {
            ...
            let spawn_slots = |mut commands: Commands| {
                commands.spawn((Node::default(), TestParent)).with_children(|parent| {
                    spawn_spell_slot(parent, SlotSource::Active, 0);
                    spawn_spell_slot(parent, SlotSource::Bag, 0);
                });
            ...
            // Both slots should have their icons visible
            let icons: Vec<_> = app
                .world_mut()
            ...
            // Both icons should be visible (both slots have spells)
            for (visibility, _) in icons {
        ...
        #[test]
        fn spell_swap_from_bag_to_active_updates_icon_visibility() {
            let mut app = setup_test_app();
            ...
            // First refresh - active is empty (hidden), bag has spell (visible)
            let _ = app.world_mut().run_system_once(refresh_spell_slot_visuals);

            // Verify initial icon visibility
            let icons: Vec<_> = app
                .world_mut()
                .query::<(&Visibility, &SpellIconImage, &ChildOf)>()
                .iter(app.world())
                .collect();
            assert_eq!(icons.len(), 2);
            ...
            // Second refresh - active has spell (visible), bag is empty (hidden)
            let _ = app.world_mut().run_system_once(refresh_spell_slot_visuals);

            // Verify the system correctly updates both slots
            // (Can't easily check which icon belongs to which slot in this test setup)
        }
    }
